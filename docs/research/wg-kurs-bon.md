...
| Источник истины | Гайд «Economy in World of Tanks»: боны в разделе Currency Exchange and Resource Conversion отсутствуют |
| Подтверждение | Статья поддержки 16135 («cannot be purchased for credits or gold»); витрина Премиум магазина без категории Bonds; Public API без коэффициента (`private.bonds`, `private.credits`, `private.gold` — все балансы) |
| Цены лотов в бонах | Не курс; в цепочку Credits → Gold → доллар не входят |
| Серый рынок / форумный «курс бон» | Не использовать |
| Боны на счету WG-аккаунта в Оценке | Не Имущество: курса-снимка нет |
| Перенос на WG договорённости Lesta «1 бона = 1 золото» | Решение по этому пункту — предмет отдельного тикета, не этого исследования |
